...
export class DrawEngine {
  ...
  private onMouseScroll = (e: WheelEvent) => {
    this.zoom = Math.max(
      1,
      this.zoom + Math.sign(e.deltaY) * Settings.DEFAULT_ZOOM_STEP
    );
    this.draw();
  };
  ...
  public drawContent() {
    this.content.forEach((object: any) => {
      if (object.hasOwnProperty('type')) {
        switch (object.type) {
          case 'circle':
            const point: IPoint = this.applyAnchorToPoint({
              x: object.x,
              y: object.y,
            });
          ...
          case 'line':
            ...
            const point2: IPoint = this.applyAnchorToPoint({
              x: object.x2,
              y: object.y2,
            });
            DrawUtil.drawLine(
